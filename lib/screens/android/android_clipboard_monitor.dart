...
class AndroidController extends BaseSifraController {
  ...
  void processClipboardContent(String content) {
    ...
    if (lines[0].isValidColor()) {
      ...
      processColorString(
          colorString,
          autoCreateColor.value,
          selectedPath.value!,
          isForRect ? ColorForLanguage.rect : ColorForLanguage.android);
  ...
  void processStringContent(
      String content, ColorForLanguage colorForLanguage) async {
    if (autoCreateString.value) {
      ...
      if (colorForLanguage == ColorForLanguage.android) {
        addStringToAndroidStringsFile(
            selectedPath.value!, content, generatedStringName);
      }
      if (colorForLanguage == ColorForLanguage.rect) {
        addStringToRectStringsFile(selectedPath.value!,
            getScreenNameForString(), generatedStringName, content);
      }
      if (colorForLanguage == ColorForLanguage.flutter) {
        addStringToRectStringsFile(selectedPath.value!,
            getScreenNameForString(), generatedStringName, content);
      }
    } else {
      ...
      await Get.dialog(
        InputNameDialog(
          ...
          onConfirm: (stringName) {
            ...
            if (colorForLanguage == ColorForLanguage.android) {
              addStringToAndroidStringsFile(
                  selectedPath.value!, content, stringName);
            }
            if (colorForLanguage == ColorForLanguage.rect) {
              addStringToRectStringsFile(selectedPath.value!,
                  getScreenNameForString(), stringName, content);
            }
          },
        ),
      );
